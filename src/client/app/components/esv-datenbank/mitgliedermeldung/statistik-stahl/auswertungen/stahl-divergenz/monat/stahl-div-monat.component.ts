...
@Component({
  selector: 'app-stahl-div-monat',
  templateUrl: './stahl-div-monat.component.html',
  styleUrls: ['./stahl-div-monat.component.css']
})
export class StahlDivMonatComponent implements OnInit {
  ...
  @Input() steelReportMonthSum: SummarizedSteelReport[] = [];
  @Input() steelReportYearSum: SummarizedSteelReport[] = [];
  ...
  async print(): Promise<void> {
    ...
    const cssFile = require('./stahl-div-monat.component.css').toString();

    const printWindow = window.open('', '', 'height=800,width=1200');

    if (printWindow) {
      const tableToPrint = document.getElementById("printSteelDivergenzMonth");
