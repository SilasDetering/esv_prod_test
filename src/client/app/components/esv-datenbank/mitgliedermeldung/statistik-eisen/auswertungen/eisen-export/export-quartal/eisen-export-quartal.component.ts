...
@Component({
  selector: 'app-eisen-export-quartal',
  templateUrl: './eisen-export-quartal.component.html',
  styleUrls: ['./eisen-export-quartal.component.css']
})
export class EisenExportQuartalComponent implements OnInit, OnDestroy {
  ...
  listOfIronReportsSelectedYear: Array<IronReport> = [];
  ...
  onSelect(): void {
    if (this.selectedQuartal != "" && this.selectedYear != "") {
      const subscription = this.reportService.getIronReportsByYear(this.selectedYear).subscribe((data) => {
        if (!data.success) {
          this.flashMessage.show(data.msg, { cssClass: 'alert-danger', timeout: 5000 });
        }
        this.listOfIronReportsSelectedYear = data.reportList;
        this.updateExports();
      })
  ...
  getCountryExportArraysFromMonth(month: number): Array<ExportReport> {
    let exports = [];
    let j = 0;
    for (let i = 0; i < this.listOfIronReportsSelectedYear.length; i++) {
      if (this.listOfIronReportsSelectedYear[i].country_exports[0] != undefined) {
        for (let l = 0; l < this.listOfIronReportsSelectedYear[i].country_exports.length; l++) {
          if (this.getReportDateMonth(this.listOfIronReportsSelectedYear[i].reportDate) == month) {
            exports[j] = this.listOfIronReportsSelectedYear[i].country_exports[l];
            j++;
          }
        }
      }
  ...
  async print(): Promise<void> {
    ...
    const cssFile = require('./eisen-export-quartal.component.css').toString();

    const printWindow = window.open('', '', 'height=800,width=1200');

    if (printWindow) {
      const tableToPrint = document.getElementById("printQuartalTotal");
