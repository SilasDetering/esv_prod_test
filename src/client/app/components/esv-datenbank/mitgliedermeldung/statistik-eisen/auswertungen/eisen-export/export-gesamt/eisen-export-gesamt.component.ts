...
@Component({
  selector: 'app-eisen-export-gesamt',
  templateUrl: './eisen-export-gesamt.component.html',
  styleUrls: ['./eisen-export-gesamt.component.css']
})
export class EisenExportGesamtComponent implements OnInit, OnDestroy {
    ...
    for (let i = 0; i < this.listOfIronReportsSelectedYear.length; i++) {
      if (this.listOfIronReportsSelectedYear[i].country_exports[0] != undefined) {
        for (let l = 0; l < this.listOfIronReportsSelectedYear[i].country_exports.length; l++) {
          if (this.getReportDateQuartal(this.listOfIronReportsSelectedYear[i].reportDate) == quartal) {
  ...
  async print(): Promise<void> {
    ...
    const cssFile = require('./eisen-export-gesamt.component.css').toString();

    const printWindow = window.open('', '', 'height=800,width=1200');

    if (printWindow) {
      const tableToPrint = document.getElementById("printTotal");
