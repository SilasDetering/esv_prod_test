...
import { ExportList, IronReport } from 'src/client/app/models/report.model';
...
@Component({
  selector: 'app-eisen-export-monat',
  templateUrl: './eisen-export-monat.component.html',
  styleUrls: ['./eisen-export-monat.component.css']
})
export class EisenExportMonatComponent implements OnInit, OnDestroy {
  ...
  setDate(event: Event) {
    ...
    // Load Reports
    let subscription: Subscription = this.reportService.getIronReportsByDate(this.helper.normDate(this.selectedDate)).subscribe(data => {
  ...
  async print(): Promise<void> {
    ...
    const cssFile = require('./eisen-export-monat.component.css').toString();

    const printWindow = window.open('', '', 'height=800,width=1200');

    if (printWindow) {
      const tableToPrint = document.getElementById("printMonthView");
