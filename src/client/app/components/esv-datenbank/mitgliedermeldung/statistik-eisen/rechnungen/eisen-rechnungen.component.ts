...
import { ReportService, IronResponse, SteelResponse } from 'src/client/app/services/esvStatistikServices/report.service';
...
@Component({
  selector: 'app-eisen-rechnungen',
  templateUrl: './eisen-rechnungen.component.html',
  styleUrls: ['./eisen-rechnungen.component.css']
})
export class EisenRechnungenComponent implements OnInit, OnDestroy {
  ...
  loadReports(): void {
    ...
    const groupMembers: Array<Member> = [];
    ...
    const subscription = forkJoin([ironObservable, steelObservable, memberObservable]).subscribe({
      next: (data: [IronResponse, SteelResponse, MemberResponse]) => {
        ...
        ironData.reportList.forEach(report => {
          listOfIronReports.push(IronReport.__fromJSON__(report));
        });

        if (!steelData.success) {
          this.flashMessage.show(steelData.msg, { cssClass: 'alert-danger', timeout: 5000 });
        }
        steelData.reportList.forEach(report => {
          listOfSteelReports.push(SteelReport.__fromJSON__(report));
        });

        if (!memberData.success) {
          this.flashMessage.show(memberData.msg, { cssClass: 'alert-danger', timeout: 5000 })
        }
        groupMembers.push(...memberData.memberList);

        // Generiere Rechnungen sobalt alle Daten geladen wurden
        this.invoice = this.invoiceService.generateIronInvoice(member, groupMembers, date, listOfIronReports, listOfSteelReports)
