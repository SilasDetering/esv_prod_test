...
@Component({
  selector: 'app-esv-import-iron-per-year',
  templateUrl: './esv-import-iron-per-year.component.html',
  styleUrls: ['./esv-import-iron-per-year.component.css']
})
export class EsvImportIronPerYearComponent implements OnInit, OnChanges, OnDestroy {
  ...
  async print(): Promise<void> {
    ...
    const cssFile = require('./esv-import-iron-per-year.component.css').toString();

    const printWindow = window.open('', '', 'height=800,width=1200');

    if (printWindow) {
      const tableToPrint = document.getElementById("printIronImportRepYear");
