...
@Component({
  selector: 'app-esv-import-steel-per-year',
  templateUrl: './esv-import-steel-per-year.component.html',
  styleUrls: ['./esv-import-steel-per-year.component.css']
})
export class EsvImportSteelPerYearComponent implements OnInit, OnChanges, OnDestroy {
   ...
   */
  loadCountryImports(){
    const subscription = this.esvService.getCountryImportsPerYear(this.selectedDate).subscribe( data => {
  ...
  entryIsNotEmpty(country: countryImport): Boolean{
    let sum = country.stahldraht_mehr_blank + country.stahldraht_mehr_verzinkt + country.stahldraht_mehr_sonstiger 
    + country.stahldraht_weniger_blank + country.stahldraht_weniger_verzinkt + country.stahldraht_weniger_sonstiger
    return sum > 0;
  }
  ...
  async print(): Promise<void> {
    ...
    const cssFile = require('./esv-import-steel-per-year.component.css').toString();

    const printWindow = window.open('', '', 'height=800,width=1200');

    if (printWindow) {
      const tableToPrint = document.getElementById("printSteelImportRepYear");
